...
这也正是 TileLang 在第 10 章里最“practical”的地方：语言本身不只是为了把算子写出来，而是为了把迁移过程、benchmark 口径和报告结论组织成一套可重复的工程闭环。只要这份报告能稳定复用，TileLang 的价值就不止停留在 kernel 层，而是进入了项目层。

从编译链路看，这份报告接的不是“手写代码本身”，而是编译器已经把结构、后端和平台参数落地后的结果。也就是说，脚本负责把 DSL 路径跑出来，报告负责把跑出来的结果固定成证据页，再回填到正文和 benchmark 里。换句话说，这一页接的是 lowering 后已经稳定下来的输出，而不是还停留在高层表达阶段的草稿。
...
换句话说，这份报告不是“项目做完后的附录”，而是 TileLang 工程闭环的一部分：正文讲方法，脚本跑结果，报告固化证据。只有三者一起存在，`practical-tilelang-dsl` 这个名字才算真正落地。
